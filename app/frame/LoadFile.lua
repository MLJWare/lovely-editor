  ...
  __call  = function (_, frame)
    ...
    local btn_yes = Button {
      ...
      mouseclicked = function ()
        ...
        local full_path = ("%s/%s"):format(edit_path.text, filename)
        if #filename == 0 or not filio:isFile(full_path) then
          app.show_popup(MessageFrame {
            title = "No such file";
            text  = ("File %q does not exist."):format(filename);
          })
        else
          frame:_try_load(filename, full_path)
        end
      end;
    }
...
function LoadFileFrame:_open_folder(folder)
  self:_load_path_directory(("%s/%s"):format(self._current_path, folder))
end

function LoadFileFrame:_try_load(filename, full_path)
  local filedata = filio:load(full_path)
  local success, message = pcall(try_invoke, self, "on_load", filename, filedata)
  if not success then
...
function LoadFileFrame:_calc_bounds()
...
function LoadFileFrame:wheelmoved(wx, wy)
...
function LoadFileFrame:_load_path_directory(path)
